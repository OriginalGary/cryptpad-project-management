...
var getDiskUsage = function (Env, Server, cb) {
    Env.batchDiskUsage('', cb, function (done) {
        var safeFolderSize = function (path) {
            return getFolderSize(path)
                .then(function (result) { return result.size; })
                .catch(function () { return 0; });
        };
        Promise.all([
            safeFolderSize('./'),
            safeFolderSize(Env.paths.pin),
            safeFolderSize(Env.paths.blob),
            safeFolderSize(Env.paths.staging),
            safeFolderSize(Env.paths.block),
            safeFolderSize(Env.paths.data),
        ]).then(function (sizes) {
            done(void 0, {
                total: sizes[0],
                pin: sizes[1],
