...
var isDangerousMethodName = function (name) {
    ...
    return dangerous.indexOf(name) !== -1;
};
...
var handleUnauthenticatedMessage = function (Env, msg, respond, Server, netfluxId) {
    ...
    Env.Log.silly('LOG_RPC', methodName);

    Env.plugins?.MONITORING?.increment(`rpc_${methodName}`);

    var method = UNAUTHENTICATED_CALLS[methodName];
