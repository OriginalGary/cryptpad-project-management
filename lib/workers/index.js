...
Workers.initialize = function (Env, config, _cb) {
    ...
    // Security: Sanitize log labels to prevent log injection attacks
    // Removes newlines and control characters that could forge log entries
    const sanitizeLogLabel = function (label) {
        if (typeof label !== 'string') { return String(label); }
        return label.replace(/[\r\n\x00-\x1f\x7f]/g, '_');
    };

    const response = Util.response(function (errLabel, info) {
        Env.Log.error('HK_DB_WORKER__' + sanitizeLogLabel(errLabel), info);
    });
